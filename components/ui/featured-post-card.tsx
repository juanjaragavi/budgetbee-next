...
export function FeaturedPostCard({
  ...
  const containerClasses =
    orientation === "horizontal"
      ? "md:flex bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow"
      : "bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col h-full hover:shadow-md transition-shadow";
    ...
    >
      <div className={imageContainerClasses}>
        <Link href={postUrl} className="block h-full relative">
          ...
          />
          {showBadge && type && (
            <div className="absolute top-0 right-0 bg-[#F7B500] text-gray-900 text-xs font-bold px-2 py-1 uppercase">
              {type}
            </div>
          )}
          {showBadge && !type && (
            <div className="absolute top-2 left-2 z-10">
              <Badge
                className={`
                ${category === "Personal Finance" ? "bg-[#F7B500] text-gray-900 hover:bg-[#E5A600]" : ""}
                ${category === "Financial Solutions" ? "bg-[#2E74B5] text-white hover:bg-[#1E5A9A]" : ""}
              `}
              >
                {category}
              </Badge>
      ...
      <div className={contentClasses}>
        ...
        <Link
          href={postUrl}
          className="block mt-1 text-lg font-semibold text-gray-900 hover:text-[#F7B500] transition-colors mb-2 line-clamp-2 text-left sm:text-left"
        >
          {title}
        </Link>

        <p className="mt-2 text-gray-700 line-clamp-3 flex-grow text-left sm:text-left">
          {description}
        </p>
