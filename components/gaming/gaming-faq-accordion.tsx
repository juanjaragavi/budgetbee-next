...
export interface FaqItem {
  ...
  question: string;
  answer: string;
  /** If true, an ad slot will render below the answer when this item is open */
  showAdOnOpen?: boolean;
  /** Ad slot id to use (e.g. "square04") */
  adSlotId?: string;
}

interface GamingFaqAccordionProps {
  items: FaqItem[];
  themeColor: string;
  defaultOpenId?: string;
}

export default function GamingFaqAccordion({
  items,
  themeColor,
  defaultOpenId,
}: GamingFaqAccordionProps) {
        ...
        return (
          <div key={item.id}>
            <button
              ...
              aria-controls={`faq-gaming-${item.id}`}
              className={cn(
